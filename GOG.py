...
with div_cols[1]:
    if st.button("Φ Whisperer Φ", key="division_diplomat"):  # Diplomat is now "Whisperer"
        st.session_state.division = "diplomat"
        st.session_state.rank = None
        st.session_state.generate = False

with div_cols[2]:
    if st.button("Σ Scribe Σ", key="division_librarian"):  # Librarian is now "Scribe"
        st.session_state.division = "librarian"
        st.session_state.rank = "Σ-X | Scribe"  # updated tag title
        st.session_state.generate = False
