...
with div_cols[1]:
    if st.button("Φ Whisperer Φ", key="division_diplomat"):
        st.session_state.division = "diplomat"
        st.session_state.rank = None
        st.session_state.generate = False

with div_cols[2]:
    if st.button("Σ Scribe Σ", key="division_librarian"):
        st.session_state.division = "librarian"
        st.session_state.rank = "Σ-X | Scribe"
        st.session_state.generate = False
...
if st.session_state.division == "combat":
    ...
    if d1.button("Φ-1", key="rank_diplomat_1"):
        st.session_state.rank = "Φ-1 | Jr. Whisperer"
        st.session_state.generate = False
    if d2.button("Φ-2", key="rank_diplomat_2"):
        st.session_state.rank = "Φ-2 | Whisperer"
        st.session_state.generate = False
    if d3.button("Φ-3", key="rank_diplomat_3"):
        st.session_state.rank = "Φ-3 | Sr. Whisperer"
        st.session_state.generate = False
...
if st.session_state.generate and st.session_state.name and st.session_state.rank:
    ...
    if div == "combat":
        ...
        idx = int(rank.split("-")[1][0]) - 1  # fixed indexing
        morph = ["TSHJSC", "TSHSC", "TSHSSC"][idx]
        rank2 = f"Serpent's Hand | Φ-{idx + 1} | {['Jr. Whisperer', 'Whisperer', 'Sr. Whisperer'][idx]}"

    elif div == "librarian":
        morph = "TSHWS"
        rank2 = "Serpent's Hand | Σ-X | Scribe"
